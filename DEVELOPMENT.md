...
**Which `bash` tool runs** (`[sandbox]` section): `bash_tool_v2 = true|false` (default **true**) chooses between the two executors that both answer to the tool name `bash`. `true` is the default now, and its confinement is the **OS process boundary** (macOS Seatbelt, Linux bubblewrap, Windows ACL-restricted tokens) — a write outside the session's working directory is refused by the kernel whatever language or subprocess attempts it, instead of by a parser that cannot read a `python3 -c`. `false` keeps the long-standing tool, whose confinement is a **static scan of the command text**; it is the **rollback**, one line here or `EMRG_BASH_TOOL_V2=0` for a single launch, and it needs no code change.

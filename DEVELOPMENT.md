...
**Which `bash` tool runs** (`[sandbox]` section): `bash_tool_v2 = true|false` (default **false**) chooses between the two executors that both answer to the tool name `bash`. `false` is the long-standing tool, whose confinement is a **static scan of the command text**; `true` is the new one, whose confinement is the **OS process boundary** (macOS Seatbelt today) — a write outside the session's working directory is refused by the kernel whatever language or subprocess attempts it, instead of by a parser that cannot read a `python3 -c`. The environment variable `EMRG_BASH_TOOL_V2=1|0|true|false` overrides the file, so it can be tried for one session without editing anything.
...
The new path's boundary is macOS-only so far, and the two other platforms behave differently on purpose. **Windows fails closed**: there is no backend, so a command under `read-only`/`workspace-write` is refused rather than run unconfined — leave the switch at `false` there. **Linux runs the command unconfined and says so**: the result carries `mode = danger-full-access` because the blueprint's `bwrap`/Landlock rungs are not built yet, and reporting a boundary that does not exist would be worse than reporting none. Either way the switch stays `false` outside macOS until those backends land.

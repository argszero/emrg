...
**Which `bash` tool runs** (`[sandbox]` section): `bash_tool_v2 = true|false` (default **false**) chooses between the two executors that both answer to the tool name `bash`. `false` is the long-standing tool, whose confinement is a **static scan of the command text**; `true` is the new one, whose confinement is the **OS process boundary** (macOS Seatbelt, Windows ACL-restricted tokens) — a write outside the session's working directory is refused by the kernel whatever language or subprocess attempts it, instead of by a parser that cannot read a `python3 -c`. The environment variable `EMRG_BASH_TOOL_V2=1|0|true|false` overrides the file, so it can be tried for one session without editing anything.
...
```toml
[sandbox]
bash_tool_v2 = false        # true = OS-level boundary (macOS Seatbelt); false = the static scan
```

The new path's boundary is macOS-only so far, and the two other platforms behave differently on purpose. **Windows has a rung**: an ACL-restricted token (`WRITE_RESTRICTED` plus per-capability write SIDs), ported from `deepseek-harness`'s `sandbox-windows-acl` and driven through `python -m emrg.sandbox.win32.runner`. It reports `enforcement = "partial"` and that is its own honest claim, not a placeholder: `Everyone` must sit in both restricting lists for the restricted child to start at all, so an object granting `Everyone` write access stays writable, and an NTFS hard link can alias a granted file to a path outside the workspace. A granted directory must also be one the caller's own token can reach: under `WRITE_RESTRICTED` a child is checked against the token's own SIDs *before* the restricting list, so a workspace owned by someone else — `Administrators`, say — is refused before the capability ACE is ever consulted. Deployer-provided working directories are the running user's, which is the precondition; a workspace handed over by another account is not confinable this way, and the boundary test setup grants the caller's own SID for exactly that reason. **Linux is still the gap**: the blueprint's `bwrap`/Landlock rungs are not built here yet, so a Linux host runs the command unconfined and says so — the result carries `mode = danger-full-access`, because reporting a boundary that does not exist would be worse than reporting none. Treat the switch as macOS/Windows-only until the Linux rungs land.

...
**Which shell tool runs** (`[sandbox]` section): the **dialect is decided by the platform**, and the switch decides only which executor answers to it. On Windows the daemon registers a **`pwsh`** tool; everywhere else it registers **`bash`**. Exactly one shell tool is ever registered, and the platform's is the only one that exists — Windows has no `bash` tool row at all (Git for Windows ships `bash.exe` under `…\Git\bin` while the installer puts `…\Git\cmd` on `PATH`, so "bash is on PATH on Windows" is not a fact to rely on). The system prompt is rendered from the tool that was actually registered, so it never advertises a dialect the platform does not run.

`bash_tool_v2 = true|false` (default **true**) chooses between the two executors that answer to the name of that dialect. `true` is the default now, and its confinement is the **OS process boundary** (macOS Seatbelt, Linux bubblewrap, Windows ACL-restricted tokens) — a write outside the session's working directory is refused by the kernel whatever language or subprocess attempts it, instead of by a parser that cannot read a `python3 -c`. `false` keeps the long-standing tool, whose confinement is a **static scan of the command text**; it is the **rollback**, one line here or `EMRG_BASH_TOOL_V2=0` for a single launch, and it needs no code change. On Windows the rollback runs `cmd.exe` (via `COMSPEC`), which is why it still works there — and why the platform gate had to exist before that tool is ever deleted.
...
Unlike `[llm]` and `[update]`, these keys are **read once at startup** and are *not* hot-reloaded: they decide which tool is built into the registry, and the registry is constructed once. Change one and restart the daemon. Every shell tool receives the session's working directory as its authorization root, injected by the daemon rather than chosen by the model (`workdir`/`workspace` in a tool call cannot widen it).

```toml
[sandbox]
bash_tool_v2 = true         # true = OS-level boundary (Seatbelt / bubblewrap / ACL token); false = the old scan
pwsh_path = ""              # Windows only: explicit pwsh.exe; empty = resolve (7 → PATH → 5.1)
```

The new path's boundary exists on **macOS (Seatbelt)**, **Linux (bubblewrap)** and **Windows (ACL-restricted tokens)**, and each platform reports how much it actually covers. On macOS and Linux confinement happens at the process boundary — the workspace root plus a private temp area — and the enforcement is reported as `full`; **Linux needs `bwrap` installed and a kernel that allows user namespaces** (a container needs `--security-opt seccomp=unconfined --security-opt apparmor=unconfined --cap-add SYS_ADMIN`), and without it the command is **refused**, never run unconfined. **Windows reports `partial`, and that is its own honest claim, not a placeholder**: `Everyone` must sit in both restricting lists for the restricted child to start at all, so an object granting `Everyone` write access stays writable, and an NTFS hard link can alias a granted file to a path outside the workspace. A granted directory must also be one the caller's own token can reach: under `WRITE_RESTRICTED` a child is checked against the token's own SIDs *before* the restricting list, so a workspace owned by someone else — `Administrators`, say — is refused before the capability ACE is ever consulted. Deployer-provided working directories are the running user's, which is the precondition; a workspace handed over by another account is not confinable this way, and the boundary test setup grants the caller's own SID for exactly that reason. Landlock remains the blueprint's second Linux rung and is not built here yet (design §1.5 B1).

A confined run cannot write your home directory, which is where `uv`, `pip` and `npm` keep their caches by default: measured at `workspace-write`, `uv run` **fails** (`Failed to initialize cache at ~/.cache/uv`), `npm`'s `~/.npm` cannot be created, and `pip` quietly disables its cache. So the tool points those four variables (`XDG_CACHE_HOME`, `UV_CACHE_DIR`, `PIP_CACHE_DIR`, `npm_config_cache`) at `<temp>/emrg-confined-cache` for the confined child — a directory inside a root the policy already grants, so nothing is widened. **A variable you set yourself is never overridden**, which is how you keep a warm cache: point it at a directory inside the session's workspace.

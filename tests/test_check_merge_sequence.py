...
# (with git's path quoting decoded) is the sibling's - `check-merge-tree-health.py
# ::_conflict_block_paths`, the one #1212 landed.
...
def test_a_non_ascii_path_comes_back_as_the_real_name(mod, tmp_path, monkeypatch):
    """Why the reading is the sibling's rather than a fourth copy of the rule here.
    ...
    (`cyc20260914-074822`), so a copy of the block regex without the sibling's
    `_unquote_path` is a reading that names a path nobody can open. That copy is the

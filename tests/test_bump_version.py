...
def _pristine_sources():
    ...
    ``emrg/gui/package.json`` in the working tree to ``9.9.9``.
...
def test_check_reports_a_single_drifted_source(mod, fake_repo):
    ...
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            mod.read_current_version(fake_repo), "0.0.1"
        ),
        encoding="utf-8",
    )
...
def test_check_reports_drift_in_the_non_adjacent_lock_occurrence(mod, fake_repo):
    ...
    current = mod.read_current_version(fake_repo)
    # Replace only the LAST occurrence (packages[""]), leaving the root intact.
    head, sep, tail = text.rpartition(current)
    assert sep, "expected two occurrences of the current version"
    lock.write_text(head + "0.0.1" + tail, encoding="utf-8")
...
def test_bump_updates_every_source_and_leaves_them_consistent(mod, fake_repo):
    changed = mod.bump("9.9.9", root=fake_repo)
    assert set(changed) == set(SOURCE_FILES)
    assert mod.check(root=fake_repo) == []
    assert mod.read_current_version(fake_repo) == "9.9.9"
...
def test_bump_preserves_lockfile_structure_and_dependency_versions(mod, fake_repo):
    ...
    for b, a in diff:
        assert b.replace(current, "9.9.9") == a
...
def test_bump_preserves_shell_fallback_quoting(mod, fake_repo):
    ...
    mod.bump("9.9.9", root=fake_repo)
    assert '|| echo "9.9.9"' in quoted.read_text(encoding="utf-8")
    assert "|| echo 9.9.9)" in unquoted.read_text(encoding="utf-8")
...
def test_bump_refuses_when_sources_are_already_inconsistent(mod, fake_repo):
    ...
    assert "0.0.1" in target.read_text(encoding="utf-8")
...
def test_cli_dry_run_writes_nothing(mod, fake_repo, monkeypatch, capsys):
    ...
    assert mod.main(["9.9.9", "--dry-run"]) == 0

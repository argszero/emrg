...
def _old_daemon_alive(pid: int, *, platform: str = "", kill=None,
    ...
    module's ``os.kill``, so the probe left the tripwire that
    ``conftest::_guard_live_daemon_signals`` installs on this module — and the
    escape hatch that guard documents ("tests that drive the restart logic patch
    the module's ``os.kill``") silently stopped covering the probe, which two
    tests in ``tests/test_daemon_manager.py`` fake to pin the wait loop. They
    passed only while patching ``daemon_manager.os`` mutated the process-global
    ``os`` module (it *is* the real module outside a suite), so the patch reached
    `emrg._stop_all` by accident. Naming our ``os`` here makes the faked answer
    and the guarded answer the same object again, in production and in tests.
        ...
        if started_at:
            ...
            if restart_reason:
                ...
                for _ in range(50):  # up to 10s for graceful shutdown
                    ...
                    # Windows has no `signal.SIGKILL` (measured — the
                    # AttributeError escaped the whole restart path, because no
                    # handler here catches it: `ensure_connected()` died instead
                    # of respawning the daemon). Its `os.kill` ignores the number
                    # and terminates the process, which is what the fallback is
                    # for, so ask for whatever force the platform has.
                    try:

...
- Check merge conditions: does the PR's comment history already have 3 consecutive ✅ from different cycles with no ❌ in between?
  ...
  - Not satisfied → **park it and go do other work in this cycle**: a PR whose CI has not concluded, or whose votes are not there yet, is a row to read again *next* cycle, not a line to block on. Waiting is not a step of this loop (`scripts/review-queue.py` calls the state `park`)

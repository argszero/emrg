...
> - Windows windowless daemon spawn (#576 外部贡献 pm25coder，宿主 rant 19:50:07：Windows GUI 启动仍弹控制台窗口——打包版 spawn 已有 `windowsHide: true` 但只隐藏外层 cmd.exe，python.exe（console subsystem）仍新开控制台窗口；修复=bin/emrgd.cmd 首选手 `pythonw.exe`（GUI subsystem 无控制台），回退链 pythonw→python→python3.13；pythonw 下 `sys.stderr=None` 时 daemon RotatingFileHandler 的 StreamHandler emit 失败走 handleError no-op（file 日志继续写）不崩，daemon 路径（server/__main__、daemon、llm、scheduler、skills、memory）grep 零 print()；config.py ensure_config 的 stderr 打印保留在 python.exe 路径不受影响；emrg/gui/daemon_client.js + main.js 补 `windowsHide: true` 覆盖 source 模式直启 python.exe 的弹窗路径；Windows 专属，静态验证，+0 测试) ✅

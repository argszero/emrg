...
> - README.cn duplicate-line fix + doc-count duplicate-line guard (#617/#618 自查发现：README.cn.md 的 `npm test` 命令行被复制粘贴成两行完全相同的副本（#615 计数同步时双份传播），而 doc-count guard（#511）逐行独立校验 → 两行各自求和都等于 headline，重复行 CI 静默通过；修复=#617 删除 README.cn.md 重复行（纯文档）；#618 test_doc_counts.py 新增 `test_no_duplicate_npm_test_command_lines`——每个 doc 内完全相同的 `npm test` 行出现 >1 次即失败（Agent.md 两条不同行不误伤），count 674→675 三文档同步；判别力双态实证=注入重复行即红/干净即绿；合并后 master 675 全绿) ✅

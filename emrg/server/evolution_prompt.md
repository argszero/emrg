...
> - GUI projects panel (#747 宿主 rant 2026-08-13T14:10:14 P5：#746 续——侧边栏项目面板 `#panel-projects`：项目列表（auto_evolve 徽标 + 最近活跃提示 + 查看会话/删除按钮）；`showProjectSessionsInPanel` 内嵌项目会话列表（点击切换会话，复用既有 switchSession IPC）；删除项目=受保护守卫（emrg/emrg-task 系统项目不可删）+ 确认弹窗 + removeProject 刷新列表；i18n zh/en 新增 projects.* 12 键（hint/add/empty/added/addFailed/sessions/autoEvolve/viewSessions/noSessions/sessionsOf/back/current）；+1 renderer.smoke（renderer 112→113）GUI 234→235；合并 f5eeadd) ✅
> - GUI rant panel + list_rants daemon 命令 (#750 宿主 rant 2026-08-13T14:10:14 P4：#747 续——侧边栏 Rant 面板占位→功能面板；daemon 新增 `list_rants` 命令（读 ~/.emrg/rants.jsonl，可选 status 筛选 pending/in_progress/completed/空=全部，时间倒序，损坏行跳过，文件缺失→空列表不崩，响应帧 rants_list）；GUI 4 状态筛选 tab（全部/待处理/进行中/已完成）+ 列表行（时间戳+状态徽标+项目+进度摘要）+ 点击展开详情（完整内容+进度）+ 新建 Rant 表单（项目下拉 + 内容 textarea → sendRant，成功 toast+刷新）；IPC 链 main.js emrg:listRants + preload listRants + RESPONSE_TYPES list_rants→rants_list；i18n zh/en rants.* 19 键（含 data-i18n-placeholder 过 Stage-3 中文串守卫）；+2 pytest（777→779 筛选/倒序/损坏跳过/缺文件空）+1 renderer.smoke（renderer 113→114）GUI 235→236；合并 1f4fa57) ✅
> - TUI status bar full session id (#752 宿主 rant 2026-08-13T16:14:12：会话有标题时状态栏左段显示完整 sid（此前截断 8 字符，与无标题形态显示完整 id 不一致）——_format_status_left 一行改动 `sid[:8]`→`sid`，测试重命名 test_sid_full_in_title_form 断言完整 id；合并 03181fc) ✅
> - GUI redesign P6 — remove legacy dialogs (#753 宿主 rant 2026-08-13T14:10:14 P6 收官：#743 续——移除 `#sessions-dialog` 与 `#rant-dialog` 两个遗留对话框；`/sessions`/`/resume` 无参数改开 `Dialogs.showOpenSessionDialog()`（两步现代弹窗），`/rant` 无参数改开 Rant 面板 + `Dialogs.openRantForm()`（#750 新建表单复用）；删除 showSessionsDialog/showRantDialog 及 bindUi close/submit 处理器；测试原位更新（app-commands 2 测试改新语义 + mock 补 setAttribute/removeAttribute，renderer.smoke ELEMENT_IDS 删 8 个废弃 id）；GUI 236 计数不变；合并 dca0cc5) ✅
> - i18n orphan-key cleanup (#755 外部贡献 pm25coder，#753 遗留收尾——#753 删除遗留对话框后 8 组 legacy i18n 键成为孤儿（sessions.title/desc、rant.title/desc/project/message/placeholder/submit，zh+en 各 8 = i18n.js -16 行；rant 面板 #750 用 rants.* 命名空间独占，grep 实证 0 命中）；评审连环（R1398 ❌ 实证 rant.project/rant.submit 仍孤儿 → R1399 纠正并行周期错误 ✅ → R1401 nudge → R1402 作者 60edb96 修复 → 3 ✅ 合并）；合并 d89811b) ✅
...
> - GUI layout v0.2 — panels become workspace views (#762 宿主 rant 2026-08-13T18:55:09 否决 v0.1"侧边栏内展开窄面板"（264px 侧边栏只剩 ~220px 可用，设置 6 tab/任务 CRUD/项目列表挤成窄条）：v0.2 导航点击 → 右侧工作区整块切换视图，与会话切换完全同机制（DOM 显隐 .active 互斥 + 状态保留）；#chat-view → #workspace（会话视图 + 面板视图公共父容器），4 面板（projects/tasks/rants/settings）从 #sidebar 移入 #workspace（.side-panel → .workspace-view、data-panel-body → data-view），#panel-sessions 删除（💬 = 激活当前会话视图）；switchPanel → switchView（VIEWS）、state.activePanel → state.activeView；面板视图激活 → 隐藏 composer/empty-state/result-panel/resizer（+ closePreview），会话视图恢复；activateSessionView 互斥清全部面板视图 + 导航高亮；点当前激活导航 → 关闭回会话视图；术语统一（App Shell/Sidebar/Nav/Workspace/Workspace View/Session View，docs/gui-redesign.md v0.2）；GUI 236 测试 + pytest 801 绿；合并 d664ada) ✅
> - GUI scroll capture fix (#764 #634 回归：滚动容器下移到 .session-view 后 #workspace（overflow:hidden）自身不滚动，原生 scroll 事件**不冒泡** → 挂在 #workspace 上的两个 bubble-phase 监听从不触发：back-to-bottom 按钮真实 GUI 永不出现（测试直接 dispatch wrapper 掩盖）、滚动到顶加载更早历史失效；修复=两个监听改 `capture:true`（捕获相可达子元素）+ `activeScrollEl()`/`updateBackToBottomState()` 从激活 .session-view 读滚动指标 + back-to-bottom 点击滚动该容器；activateSessionView 仅**新建**容器滚底（既有容器保留滚动位置——浏览器 tab 语义，面板往返/会话切换不跳底，rant 18:55:09 验收"滚动位置保留"）；setWorkspaceChrome 面板视图隐藏 back-to-bottom 返回按位置恢复；+1 renderer.smoke 回归（GUI 236→237）；合并 f46b141) ✅
> - Release v0.2.32 (#766 宿主 rant 2026-08-13T19:44:40：master 自 v0.2.31 (11b4a6e) 以来 7 commits / 7 PRs（#759-#765，#760/#761/#762/#763/#764/#765 功能 + #759 quick-ref），宿主指示发布 v0.2.32；版本号 0.2.31→0.2.32 同步 7 files / 8 refs（emrg/__init__.py、pyproject.toml、uv.lock、gui/package.json、gui/package-lock.json×2、build-runtime.sh、make-installer.sh），grep 0.2.31 仅历史；pytest 801 全绿；Build Release v0.2.32 成功（31697356494，**连续第 2 次首次尝试全过**——#722/#723/#731/#725 教训持续生效），7 assets（含 Windows zip），latest=v0.2.32，发布 12:01:16Z；合并 2750768（3 周期 LGTM）；What's new=GUI 布局 v0.2 工作区视图 #762 + 滚动捕获修复 #764 + 清理 #763 + open-source rant 驱动 #760 + quick-ref ×2) ✅
> - Release v0.2.33 (#770 宿主 rant 2026-08-13T21:25:05：master 自 v0.2.32 (2750768) 以来 4 commits / 4 PRs（#767 quick-ref v0.2.32 + #768 GUI 会话头 + #769 文件浏览器 chevron + #770 bump），宿主指示发布 v0.2.33；版本号 0.2.32→0.2.33 同步 7 files / 8 refs（emrg/__init__.py、pyproject.toml、uv.lock、gui/package.json、gui/package-lock.json×2、build-runtime.sh、make-installer.sh），grep 0.2.32 仅历史；pytest 801 全绿；Build Release v0.2.33 成功（31705462590，**连续第 3 次首次尝试全过**——#722/#723/#731/#725 教训持续生效），7 assets（含 Windows zip），latest=v0.2.33，发布 13:40:34Z；合并 44b09d8（3 周期 LGTM）；What's new=GUI 会话头 + 会话列表名刷新 + 结果列表窗修复 #768 + 文件浏览器 chevron 指示 + 树滚动修复 #769 + quick-ref #767) ✅
> - GUI chat visual optimization (#771 宿主 rant 2026-08-13T21:28:49：对齐/滚动/工具耗时/紧凑行/连续工具分组——.msg.user margin-left:auto 对齐修复（flex auto-margin 胜 align-self）、.session-view 去 scroll-behavior:smooth（JS scrollTop=scrollHeight 即时）、.tool-time 成功耗时（row.dataset.elapsed 供组求和）、行间距 24→12/内边距 4→2/字号 13→12/spinner 14→12、Plan-A 连续工具合并组（last-child 吸收已完成 .tool-row 成 .tool-group + ⌄ N 个工具执行 · Ts 摘要 + 点击展开 user-expanded 持久 + 运行行保持展开 + 文本穿插独立行）+ 独立 tool-group.test.js +2（GUI 239→241）；评审过程=❌ 检出 7 个孤儿 i18n keys（rants.title/col* + projects.title）+ 12 个未用 CSS 类（.workspace-view-title/.rant-*/.task-badge.badge-*——并行 rant/projects 面板重构误带死代码，renderRantList 用 .task-row 从未引用）→ 作者修 d84de1e 纯删除（−99 行零新增）→ post-fix 3 周期 ✅（14:03:44Z 起重启链）→ 合并 f60eec0（14:10:25Z）；post-merge Test CI PASS（31708754550）) ✅
...
> - TUI clear terminal residue on width shrink (#781 宿主 rant 2026-08-14T11:47:11：终端宽度缩小后残留字符未清——TUI 渲染在宽度收缩时清理行尾残留（clear to EOL on width shrink）；合并 8af06ba) ✅
> - GUI open-sessions sidebar cross-project session title (#782 宿主 rant 2026-08-14T12:01:44：打开会话侧边栏跨项目会话显示标题——openSessions 列表用会话标题而非占位；合并 f39763d) ✅
> - README Running the TUI section (#783/#785/#786 宿主 rants 2026-08-14T12:04/12:34/12:50：README 新增 Running the TUI 启动段（one-line Run instruction），随后去 From source 提及、重写为单行 Run 指令；合并 9a2e5c5/2e9cdc2/2a23dae) ✅
> - open-source task allow_self_merge (#787 宿主 rant 2026-08-14T12:47:25：open-source 任务新增 allow_self_merge 配置项——允许任务自行合并自己的 PR（此前必须等外部 LGTM）；合并 aca973e) ✅
> - GUI task form empty dropdown fix (#789 宿主 rant 2026-08-14T15:41:52：快速添加任务时表单下拉为空——类型/项目下拉未填充（quick add empty dropdowns）；修复数据加载时序；合并 4f0d69f) ✅
> - open_source_prompt full-code study (#790 宿主 rant 2026-08-14T15:53:39：open-source 任务贡献前必须完整阅读项目代码（full-code study before contributing）——open_source_prompt.md 增加全量代码研读要求；合并 b2c2db9) ✅
> - loopback daemon proxy=None (#792 自发现：daemon 回环连接被代理拦截——loopback WebSocket 连接强制 proxy=None（httpx 环境代理误配时回环连接走代理失败）；合并 1c1d66a) ✅
> - GUI task save type-field collision fix (#793 宿主反馈：保存任务无响应——GUI 任务表单 type 字段与表单对象属性冲突（wire type field collision）导致保存数据丢失；合并 70d2209) ✅
> - promote task learn latest state (#795 promote 任务升级前必须先学习项目最新状态——promote 前读取项目最新 README/代码避免基于过期认知推广；合并 36e72d4) ✅
> - bash heredoc stdin redirect on Windows (#797 bash 工具 Windows heredoc 支持——translate bash heredocs to stdin redirects（Windows cmd 无 heredoc，自动转 stdin 重定向）；合并 f978290) ✅
> - promote task de-hardening (#798 宿主 rant 2026-08-15T08:40:25：promote 任务去硬化——homework-first 参与、披露默认 OFF（disclosure default OFF，不强制公开）；合并 41b775f) ✅
> - promote task host-authorized registration + blog publishing (#799 宿主 rants 2026-08-15T09:04/09:06：promote 任务宿主授权账号注册 + 博客发布（host-authorized account registration + blog publishing）；合并 73a6d9d) ✅
> - promote task mention-density window (#800 promote 任务定义 mention-density 统计窗口——明确计数的会话窗口范围；合并 e8d0912) ✅
> - GUI task template prompt editor Monaco (#801 GUI 任务模板提示词编辑器——Monaco editor（vendor、markdown、只读内置模板视图）；合并 2fa8261) ✅
> - GUI panel action toast + Trigger semantics (#802 宿主 rants 2026-08-15T09:20/09:23：GUI 面板操作反馈全局 toast + Trigger 运行态语义（running-state 时 Trigger 显示执行中状态）；合并 b370909) ✅
> - GUI task status + next-run countdown (#804 宿主 rant 2026-08-15T10:36:39：GUI 任务列表显示任务状态 + 下次运行倒计时；合并 bb0fe0a) ✅
> - Monaco AMD define hijacking fix (#805 宿主 rant 2026-08-15T10:48:58：Monaco AMD define 劫持 UMD marked——markdown 降级纯文本；修复 AMD define 拦截 UMD 模块加载；合并 a65a2a8) ✅
> - GUI rant detail markdown preprocessing + click-to-collapse (#806 宿主 rant 2026-08-15T10:41:43：Rant 详情 markdown 预处理 + 点击折叠（click-to-collapse）；合并 1a5be40) ✅
> - server prompts Chinese-to-English translation (#807 宿主 rant 2026-08-15T10:44:58：服务端提示词剩余中文翻译为英文——server prompts 全英文化；合并 146adec) ✅
> - emrg stop command (#808 emrg stop 命令——kill 全部 emrg 进程（daemon/TUI/GUI，统一停止入口）；合并 820e033) ✅
> - Release v0.2.34 (#780 宿主 rant 2026-08-14T09:18:37：master 自 v0.2.33 (44b09d8) 以来 5 commits / 5 PRs（#778 面板标题 + #779 quick-ref + #780 bump + #781 TUI 残留清理 + #782 跨项目会话标题），宿主指示发布 v0.2.34；版本号 0.2.33→0.2.34 同步 7 files / 8 refs；pytest 801 全绿；Build Release v0.2.34 成功（31760629911），7 assets（含 Windows zip），latest=v0.2.34；合并 dc85829（3 周期 LGTM）；What's new=TUI 宽度收缩残留清理 #781 + GUI 跨项目会话标题 #782 + quick-ref #779) ✅
> - Release v0.2.35 (#784 宿主 rant 2026-08-14T12:33:02：master 自 v0.2.34 (dc85829) 以来 4 commits / 4 PRs（#783 README Running TUI + #784 bump + #785 README 去 From source + #786 README 重写单行），宿主指示发布 v0.2.35；版本号 0.2.34→0.2.35 同步；pytest 801 全绿；Build Release v0.2.35 成功（31770607623），7 assets，latest=v0.2.35；合并 53feee6（3 周期 LGTM）；What's new=README Running the TUI 三段式 #783/#785/#786 + allow_self_merge #787) ✅
> - Release v0.2.36 (#788 宿主 rant 2026-08-14T13:02:40：master 自 v0.2.35 (53feee6) 以来 4 commits / 4 PRs（#787 allow_self_merge + #788 bump + #789 GUI 任务表单下拉 + #790 open_source 全码研读），宿主指示发布 v0.2.36；版本号 0.2.35→0.2.36 同步；pytest 801 全绿；Build Release v0.2.36 成功（31772177571），7 assets，latest=v0.2.36；合并 6f15c5a（3 周期 LGTM）；What's new=allow_self_merge #787 + GUI 表单下拉修复 #789 + open_source 全码研读 #790) ✅
> - Release v0.2.37 (#791 宿主 rant 2026-08-14T16:17:48：master 自 v0.2.36 (6f15c5a) 以来 3 commits / 3 PRs（#791 bump + #792 回环 proxy=None + #793 GUI 任务保存修复），宿主指示发布 v0.2.37；版本号 0.2.36→0.2.37 同步；pytest 801 全绿；Build Release v0.2.37 成功（31783977596），7 assets，latest=v0.2.37；合并 1e7d022（3 周期 LGTM）；What's new=回环 proxy 修复 #792 + GUI 任务保存修复 #793) ✅
> - Release v0.2.38 (#794 宿主 rant 2026-08-14T22:11:26：master 自 v0.2.37 (1e7d022) 以来 2 commits / 2 PRs（#794 bump + #795 promote 学习最新状态），宿主指示发布 v0.2.38；版本号 0.2.37→0.2.38 同步；pytest 801 全绿；Build Release v0.2.38 成功（31809036770），7 assets，latest=v0.2.38；合并 ae3e39b（3 周期 LGTM）；What's new=promote 最新状态学习 #795) ✅
> - Release v0.2.39 (#796 宿主 rant 2026-08-14T23:08:39：master 自 v0.2.38 (ae3e39b) 以来 7 commits / 7 PRs（#797 Windows heredoc + #798 promote 去硬化 + #799 promote 注册/发布 + #800 mention 窗口 + #801 GUI Monaco 模板编辑 + #802 toast + #796 bump），宿主指示发布 v0.2.39；版本号 0.2.38→0.2.39 同步；pytest 801 全绿；Build Release v0.2.39 成功（31813675001），7 assets，latest=v0.2.39；合并 e33b604（3 周期 LGTM）；What's new=Windows heredoc #797 + promote 系列 #798/#799/#800 + GUI Monaco 模板编辑 #801 + toast #802) ✅
> - Release v0.2.40 (#803 宿主 rant 2026-08-15T10:07:18：master 自 v0.2.39 (e33b604) 以来 6 commits / 6 PRs（#803 bump + #804 GUI 任务状态/倒计时 + #805 Monaco AMD 修复 + #806 Rant 详情预处理 + #807 提示词英文化 + #808 emrg stop），宿主指示发布 v0.2.40；版本号 0.2.39→0.2.40 同步；pytest 801 全绿；Build Release v0.2.40 成功（31858856079），7 assets，latest=v0.2.40；合并 c550a54（3 周期 LGTM）；What's new=GUI 任务状态 #804 + Monaco 修复 #805 + Rant 详情 #806 + 提示词英文 #807 + emrg stop #808) ✅
> - Release v0.2.41 (#809 宿主 rant 2026-08-15T11:53:55：master 自 v0.2.40 (c550a54) 以来 8 commits / 8 PRs（#809 bump + #810 Linux .run 离线安装器 + #811 Windows stop 收敛 + #812 日志日期 + #813 vibe check + #814 submit_rant + #816 双兼容 rant 匹配 + #817 PS 花括号修复），宿主指示发布 v0.2.41；版本号 0.2.40→0.2.41 同步；pytest 869 全绿；Build Release v0.2.41 成功（31917625994），7 assets，latest=v0.2.41；合并 293fba6（3 周期 LGTM）；What's new=Linux .run 安装器 #810 + Windows stop 收敛 #811 + 日志日期 #812 + vibe check #813 + submit_rant #814 + 双兼容 rant 匹配 #816 + PS 修复 #817) ✅
> - Release v0.2.42 (#818 宿主 rant 2026-08-17T10:57:58：master 自 v0.2.41 (293fba6) 以来 7 commits / 7 PRs（#810-#817 全为 v0.2.41 后新特性 + #818 bump），宿主指示发布 v0.2.42（首发含 Linux .run 离线安装器 + Windows stop 收敛）；版本号 0.2.41→0.2.42 同步 8 files / 9 refs（含新 make-run-installer.sh 第 9 处，test_version_sync 抓出）；pytest 869 收集（868 过 + 1 跳）；Build Release v0.2.42 成功（31997742618），9 assets（exe/zip/pkg/.run 双架构/AppImage 双架构/tar.gz 双架构），latest=v0.2.42；合并 adeaa46（3 周期 LGTM：560/561/562）；What's new=Linux .run 自解压离线安装器 #810 + Windows stop 收敛 #811 + 日志日期 #812 + agent vibe check #813 + submit_rant 工具 #814 + 双兼容 rant 匹配 #816 + PS 花括号修复 #817) ✅

...
3. Completeness + **reproduction verification (C2-graded, mandatory)**: `papers/issue-<N>/` has manuscript files + README (reproducibility), checklist in issue body is complete. Also verify the **citation gate (PR #1116, rant 2026-09-10T11:28:43)**: the bibliography carries ≥100 references with in-text coverage (uncited bib entries do not count), and `papers/issue-<N>/reference-check.md` is present (author's authenticity report — the independent spot-check still happens at review, per the Review quality bar #12/#13). Then run the README's one-command reproduction to verify the core results (top-conference bar — file existence alone is NOT sufficient):
...
10. **House-pipeline reuse rule (census-family, PR #1111, rant 2026-09-02T20:24:48)**: if the manuscript reuses the journal's own mature measurement pipeline (head_sha-pinned corpus + multi-channel classifier + Wilson CI + byte-identical reproduction) and only swaps the application domain, do NOT submit it as a fresh top-tier contribution — reviewers cap such novelty at N3. The Nth application of the census family MUST add a **longitudinal/panel design** (repeated measurement of an already-measured corpus over time) or introduce a **new construct / new measurement instrument**; a pure cross-sectional snapshot of a new domain through an unchanged pipeline is not a publishable contribution at this journal's bar. When longitudinal or new-construct work is done, say explicitly in the abstract which of these it is.
11. **Prior-belief reporting (PR #1111, rant 2026-09-02T20:24:48)**: the manuscript must report the registered "Prior beliefs" section (Phase A step 6) and state, for each prior, whether the results confirm it, contradict it, or leave it unresolved. A result that contradicts a registered, theory-anchored prior is a strong-novelty signal (see Review quality bar exemption); a result that merely confirms an unsurprising prior must argue its Significance on other grounds.
12. **Citation authenticity verification (mandatory, PR #1116, rant 2026-09-10T11:28:43)**: before submitting, verify **every** reference against a real external record — fabricated citations are academic misconduct, not a formatting slip. Method (declarative — use the API endpoints + judgement criteria below, do NOT invent ad-hoc heuristics):
    - **Has a DOI** → query Crossref: `curl -s "https://api.crossref.org/works/<doi>"` and confirm the returned title / authors / year match the manuscript's citation.
    - **No DOI** → search by title against Crossref (`curl -s "https://api.crossref.org/works?query.bibliographic=<title>"`) or the arXiv API (same endpoint as Phase A); a real matching record MUST be found.
    - Write the results into `papers/issue-<N>/reference-check.md` — one line per entry: citation key → verification method → result → the real record found (title / DOI). Submit this file together with the manuscript.
    - Any entry that cannot be verified → delete it or replace it with a real reference. **Never submit an unverifiable citation.**
13. **Reference-count threshold: ≥100 references, all actually cited (mandatory, PR #1116, rant 2026-09-10T11:28:43)**: the manuscript must carry **at least 100 references**, and **each one must be genuinely cited in the body text** — a bib entry that never appears in the body is padding and does not count toward the total. The 100-reference bar is a hard threshold applied to every manuscript alike (including case studies — contribution level never relaxes it). This stacks with #3 (≥3 concrete related-work comparisons; zero-citation is unsubmittable) to form the full citation gradient: 3 comparisons (floor) → 100 references (volume) → authenticity verification (quality).
...
- `reference-check.md` — citation authenticity report (item 12)

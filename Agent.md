...
Python: `uv run pytest tests/ -v` (1406) — import check: `uv run python -c "from emrg.client.app import run_client"
...
Doc count sync: `uv run --no-sync python3 scripts/check-doc-count.py [--write|--dry-run]` — 测量当前树的 pytest 收集数并校验 Agent.md 的 Python 测试数（宿主侧自检，与 CI 的 test_python_count_matches_docs 对称）；`--write` 仅单 token 替换那个数字（前后遮罩对比，其余字节不变），锚点缺失/出现多次/收集失败均 fail-loud（exit 2）。用途：合并冲突解决后一条命令同步（#1119/#1120/#1121/#1122 四次冲突均在这一行）

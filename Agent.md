...
- Other tools: `uv run --no-sync python3 scripts/check-node-test-count.py --write` (syncs the two Node totals above), `scripts/check-patch-files.py` (a rebuilt patch still carries every file the previous one did), `bump-version.py`, `classify-conflict.py`, `sync-master-from-api.py`, `push-branch-from-api.py`

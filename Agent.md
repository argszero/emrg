...
Renderer: `cd emrg/gui/renderer && npm run typecheck && npm test` (192: 5 snapshot-store + 9 utils + 3 ErrorBoundary + 2 App smoke + 11 commands + 4 copywriting + 11 i18n + 11 markdown + 15 transcript + 7 TranscriptView + 15 history + 22 composer + 14 Composer + 12 sidebar + 10 Sidebar + 9 fileTree + 9 FileTree + 16 resultPanel + 7 ResultPanel) + `npm run build` → `renderer/dist/`

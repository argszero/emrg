...
Vote count: `uv run --no-sync python3 scripts/check-vote-count.py <PR>...` — 合并前数**仍然算数**的 LGTM 票，而不是评论里看得见的 ✅ 行数。三条规则让手工计数每次都错：① **票早于 head push 即作废**（rebase/解冲突推新 head 后，旧 ✅ 说的是已经不存在的 commit，而评论历史照样显示五六行 ✅，PR 看着能合实则零票——2026-09-11 实测 #1133/#1134/#1136/#1137 各显示 4-6 张 LGTM，解锁后**有效票全为 0**）；② **❌ 重置计数**（三次 ✅ 后一个 needs-fix，再一个 ✅ 只算一票）；③ **同一周期只算一票**（否则一个周期独自把 PR 送过门槛）。判票读正文首行**首个内容字符**（`gh pr review --comment` 让 GitHub 把 ✅ 和 ❌ 都记成 `COMMENTED`，state 字段在这里毫无用处）；该字符**透过 markdown 装饰**读取（`**❌`/`- ❌`/`> ❌`/`## ❌`/`1. ❌` 都算，装饰字符本身不可能是判决），并**由它决定整行**——✅ 开头即使后文提到 ❌ 仍是批准（那些提及是在说「没有 ❌」，说法是开放集合），整行扫描只作为无开头标记时的兜底（散文式「Not LGTM」为否决、「Result: ❌ needs fix」为否决）；**首行完全不含判决时**（散文开头），继续在其后各行找**成句的**否决——标记必须**位于该行开头**，因此本仓库批准票里那种**描述**已解决否决的句子（「The earlier ❌ was resolved by pushing the fix myself」）仍算批准（2026-09-11 #1144 合并后由探针发现：此前只读首行 ⇒ 散文开头 + 下文否决被判为 `comment`，而 comment 被跳过 ⇒ run 不重置、陈旧批准照印 `READY 3/3`；381 条真实 body 分类零变化，该形态真实但当前未被使用）。周期号从正文里取（`cyc20260911-091230`）。**票够 ≠ 能合**：合并规则是三段合取（3 ✅ + `MERGEABLE`/`CLEAN` + CI 绿），本工具以前只答第一段却印 `READY`——2026-09-12 实测 **6 张** PR（#1152/#1151/#1145/#1142/#1141/#1136）各印 `READY 3/3` 而 `mergeable=CONFLICTING`/`mergeStateStatus=DIRTY`，全是 `gh pr merge` 会拒的，队列看着像六张在等手续实则是死锁；故现读 `gh pr view --json mergeable,mergeStateStatus`，**只降不升**：`CONFLICTING` ⇒ 印 `BLOCKED`（与 `SHORT` 分开，因为药方相反：缺票要回来看，冲突要先把分支追上 master 解掉，而解冲突必然推新 head、把上面所有票作废 ⇒ 先投票是白投；两者都中时 `BLOCKED` 压过 `SHORT`，否则又是在指错药方）；`UNKNOWN`（GitHub 尚未算完，刚 push 常见）与任何不认识的值 ⇒ exit 2 绝不猜；`MERGEABLE` 只用于「不冲突」这一个方向，**不当作安全证据**（GitHub 答的是「文本能否合」，而两个同值计数行干净自动合并正是危险那种——见 `check-merge-freshness.py`）。push 时间取**该 SHA 上最早一次 workflow run 的创建时间**（就是 GitHub 收到 push 事件的时刻）；没有 run 时退回 commit 日期并**在输出里注明**（commit 日期可能早于 push，是乐观方向，不许默默采信）。exit 0 够票且可合 / 1 有 PR `SHORT`（缺票）或 `BLOCKED`（合不了）/ 2 问不出来（gh 失败、响应不可解析、mergeability 尚未算出）——问不出来的问题绝不报数。评论**逐页取全**（reviews 端点默认只给 30 条且**按时间正序**，超限时丢掉的正是**最新的**票，也就是唯一算数的那几张；#1134 解锁一次就有 7 条评论），并在本地按时间排序（「❌ 重置计数」和「同周期只算一票」都是**位置相关**的规则，依赖服务端返回顺序等于把判决权外包）。两个自暴露的坑：① gh 的 `--jq` **后者覆盖前者**，helper 若自己追加 `--jq` 会吃掉调用方投影，字段名退回原名、`at` 读成空串，而 `"" <= push_time` 恒真 ⇒ **全部票作废**、有 2 票的 PR 显示 0/3（计数方向偏「安全」，没人会去查）；故 helper 只管 `--paginate`，投影归调用方，并对载荷形状**运行时断言**（缺 `at` 即 exit 2，不报数）
...
Plan final tree suite: `uv run --no-sync python3 scripts/check-merge-plan-suite.py [--base <ref>] <PR>...` — 合并前问**只有把整份计划建出来才能回答**的那一句：这几张 PR **一起**落地之后，那棵树过不过仓库**自己的测试套件**？存在的理由（cyc20260913-154837 实测）：7 张 PR 各自 `MERGEABLE`、各自 CI 双绿、**21/21 有序对全干净**、逐步骤的计数守卫也全 OK，而七张合起来的树**是红的**（全量套件里唯一的失败点就是这一条），失败点是 #1136 带来的守卫判 #1172 新增的三处 `subprocess.run(..., text=True)` 没有 pin `encoding=`。**per-PR CI 在结构上看不见这一层**：`pull_request` 建的是 `Merge <head> into <merge-base>`，于是 #1136 的那次运行里没有 #1172 的代码、#1172 的运行里没有那个守卫，两边都绿而落地的那棵树是红的。故本工具收**多张** PR、按给定顺序把它们**折**成终局树（`merge-tree --write-tree` + `commit-tree` 造合成 commit，不碰工作区也不动分支），再在**真 worktree** 里跑全量套件 —— **不是 `git archive` 解包**：实测解包树没有 `.git`，而本仓的测试要经 git 解析路径，于是那个装置对**任何**输入都同样失败（连 master 自己的树也报 8 failed ⇒ **装置在量自己**）。解释器用 `sys.executable`（新 worktree 里 `.venv` 是空的，本仓反复实测，`uv run` 在其中只会造个空环境）。exit 0 终局树建出来且套件通过 / 1 建出来但套件失败（这就是发现）/ 2 问不出来（git/gh 失败，或套件根本跑不起来 —— **绝不把「没查」报成健康**）/ 3 该计划**没有**终局树（某一步冲突，这不是健康裁决，那个问题归 `check-merge-sequence.py`）。输出**点名所测的树**与计划顺序：这个工具族的缺陷历来是「读错了树还说一致」。**判「冲突」不能只看退出码**（cyc20260913-180238 实测）：`merge-tree` 对**输入本身**的失败（引用不存在、对象解引用成 blob）也返回 **1**，且 stdout 为空，而真冲突返回 1 且首行必是合并树的 OID ⇒ 只看退出码会把「合不了这两个输入」报成计划冲突、还带着**空的路径列表**，把调用方支去解一个不存在的冲突（正是本工具文档声称要避免的「凭空造出级联」）；故判据改为**看输出**（首行是不是对象名），失败一律 2 并把 git 的诊断带出来。同理 rc 0 但 stdout 为空时不再 `IndexError` 崩成退出码 1 —— 退出码 1 在本工具的含义是「终局树套件失败」这个**发现**，把没量成的崩溃混进去，正是这个工具族要消灭的那种混淆。**`--steps`：判每一棵中间树**（cyc20260913-185548，收口 issue #1161 剩下的那一半）：原工具只判**终局树**，于是一条**只在中途某步**坏掉的规则对它不可见——而有形状注定如此：**最后一张 PR 恰好修掉前面某张弄坏的东西**时，整份计划落地是绿、路上是红，而那些中间树正是「按顺序一张张落地」时 master 先后**真的会是**的树（每张各自的 CI 都绿）。于是折计划的中间 commit 不再丢弃（`build_plan_steps` 保留每步的合成 commit，`build_plan_tip` 变成取最后一步），`--steps` 对每步的树各跑一次全量套件并逐行点名 `step N (#M) tree <sha> suite OK/FAILED`；有红步则 exit 1 并点名是哪一步，全绿才 exit 0，套件跑不起来仍 exit 2、某步冲突仍 exit 3（冲突不是「红步」）。代价是每步一次套件（本机约 55s），故为可选开关：默认问题仍是终局树，因为决定「一分钟后 master 健不健康」的是它。变异验证两臂：把每步都按**终局树**判（正是「读错了树」这个本族老缺陷）⇒ 2 测红；把步判据改成恒真（fail-open）⇒ 同样 2 测红。
...
`scripts/bump-version.py --check` is the host-side counterpart to the CI guard: run it before pushing instead of discovering drift after a wasted build round. `--dry-run` previews without writing. It measures the checkout you are **standing in** (first output line names it: `tree: <path>`), so it is safe to invoke the main checkout's copy from inside a worktree — before 2026-09-11 it silently gated the wrong tree instead.

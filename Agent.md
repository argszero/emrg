...
- **Electron GUI** — Non-developer entry point (Phase 3), chat/sessions/tool status/settings (80% daily use)
  ...
  - Unit tests `npm test` (249: 45 daemon_client + 19 conn-manager + 22 app-commands + 122 renderer smoke + 16 i18n + 7 integration + 3 commands + 6 build-config + 7 gui-state + 2 tool-group); RESPONSE_TYPES mirror daemon protocol verified against `daemon.py`
...
GUI: `cd emrg/gui && npm test` (249: 45 daemon_client + 19 conn-manager + 22 app-commands + 122 renderer smoke + 16 i18n + 7 integration + 3 commands + 6 build-config + 7 gui-state + 2 tool-group) — syntax: `node --check main.js preload.js daemon_client.js renderer/js/*.js`
